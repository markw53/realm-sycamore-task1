  ...
  async execute(job: Job): Promise<JobResult> {
    ...
    try {
      if (action === "echo") {
        ...
        await sleep(delay);
        return JobResult.ok(job.id, { echoed: message });

      } else if (action === "transform") {
        const data = (job.payload.data as Record<string, unknown>) ?? {};
        const operation = (job.payload.operation as string) ?? "identity";
        return JobResult.ok(job.id, transform(data, operation));

      } else if (action === "aggregate") {
        const items = (job.payload.items as unknown[]) ?? [];
        return JobResult.ok(job.id, aggregate(items));

      } else {
        return JobResult.err(job.id, `Unknown IO action: ${action}`);
      }
    } catch (e: any) {
